...
void print(char *fmt, ...) {
  ...
  while(*cur) {
    if(*cur == '%') {
      cur++;
      switch(*cur++) {
        case 'd': printint(va_arg(va, ll)); break;
 ...
 * @file The file name
*/
void error(char *message, ll line, char *file) {
  print("%s: %s, line #%d\n", message, file, line);
  exit(1);
}
...
*/
void Writer_io_write(ll* out, int ch) {
  char c = ch;
  write(1, &c, 1);  
  out[REF]--;
}
 ...
 * @file The file name
*/
void nullexception(ll line, char *file) {
...
*/
void inheritexception(ll line, char *file, char *to, char *from) {
...
*/
ll *addarray(ll vtable, ll classID, ll n, ...) {
  ll dim[n];
  ll size = 1;
  ...
  ll *array = (ll*) calloc(size + n + RESERVED, 8);
  allocated += (size + n + RESERVED) * 8;
  array[VTABLE] = vtable;
  array[CID] = -classID;
  array[SIZE] = size;
  array[DIM] = n;
...
*/
ll indexarray(ll line, char *file, ll *array, ll pointer, ll n, ...) {
  ...
  va_start(va, n);
  for(int i = 0; i < n - 1; i++) {
    idx += va_arg(va, ll) * array[DIM + i + 1];
  }
  idx += va_arg(va, int);
  va_end(va);
...
*/
ll *addobject(ll vtable, ll size) {
  ll *object = (ll*) calloc(size + RESERVED, 8);
  allocated += (size + RESERVED) * 8;
...
*/
void destroyobject(ll *object) {
  if(object[CID] <= 0) {
    if(object[CID] < 0) { // Class Type
      int start = RESERVED + object[DIM];
      for(int i = start; i < object[SIZE] + start; i++) {
        if(object[i]) { calldestroy(object[i]); } 
      }
    }
    freed += (RESERVED + object[DIM] + object[SIZE]) * 8;
  } else {
    freed += (object[SIZE] + RESERVED) * 8;
  }
  free(object);
  object = NULL;
}
...
*/
 ll* createstring(char *lit, ll vtable) {
  ll *newStr = addobject(vtable, 1);
  Stringinit(newStr);
  while (*lit) {
    newStr[REF]++;
